        ...
        <div>
          <Link to='/'>
            <div className="flex items-center w-60">
              <img className="w-10" src={logov} alt='logo' />
        ...
        <div className='ml-6'>
          <ul className="flex items-center text-sm font-bold">
            <li onClick={(e) => rippleEffect(e)} className='relative overflow-hidden'>
              <Link to='/stake'><div onClick={() => setActiveTab('stake')} className={`inline-block px-4 py-8 hover:text-white transition-all ${activeTab === 'stake' ? 'text-white': 'text-nav-link'}`}>Stake</div></Link>
              {activeTab === 'stake' && indicator}
            </li>
            <li onClick={(e) => rippleEffect(e)} className='relative overflow-hidden'>
              <Link to='/claim'><div onClick={() => setActiveTab('claim')} className={`inline-block px-4 py-8 hover:text-white transition-all ${activeTab === 'claim' ? 'text-white': 'text-nav-link'}`}>Claim</div></Link>
              {activeTab === 'claim' && indicator}
            </li>
            <li onClick={(e) => rippleEffect(e)} className='relative overflow-hidden'>
              <Link to='/lock'><div onClick={() => setActiveTab('lock')} className={`inline-block px-4 py-8 hover:text-white transition-all ${activeTab === 'lock' ? 'text-white': 'text-nav-link'}`}>Lock</div></Link>
              {activeTab === 'lock' && indicator}
            </li>
            <li onClick={(e) => rippleEffect(e)} className='relative overflow-hidden'>
              <div onClick={(e) => rippleEffect(e)} className='inline-block cursor-pointer px-4 py-8 text-nav-link hover:text-white transition-all'>Menu</div>
            </li>
          </ul>
        </div>
      </div>
